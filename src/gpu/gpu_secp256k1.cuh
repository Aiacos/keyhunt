/*
 * GPU secp256k1 implementation for CUDA
...
__device__ __constant__ uint256_t SECP256K1_P = {{
    0xFFFFFC2Fu, 0xFFFFFFFEu, 0xFFFFFFFFu, 0xFFFFFFFFu,
    0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu
}};

// secp256k1 order: n
__device__ __constant__ uint256_t SECP256K1_N = {{
    0xD0364141u, 0xBFD25E8Cu, 0xAF48A03Bu, 0xBAAEDCE6u,
    0xFFFFFFFEu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu
}};

// Generator point G (affine)
__device__ __constant__ uint256_t SECP256K1_GX = {{
    0x16F81798u, 0x59F2815Bu, 0x2DCE28D9u, 0x029BFCDB,
    0xCE870B07u, 0x55A06295u, 0xF9DCBBACu, 0x79BE667Eu
}};

__device__ __constant__ uint256_t SECP256K1_GY = {{
...
__device__ __forceinline__ void u256_set_zero(uint256_t *r) {
    #pragma unroll
    for (int i = 0; i < 8; i++) r->d[i] = 0;
}

__device__ __forceinline__ void u256_set(uint256_t *r, const uint256_t *a) {
    #pragma unroll
    for (int i = 0; i < 8; i++) r->d[i] = a->d[i];
}

__device__ __forceinline__ void u256_set_u32(uint256_t *r, uint32_t v) {
    r->d[0] = v;
    #pragma unroll
...
// r = a + b, returns carry
__device__ __forceinline__ uint32_t u256_add(uint256_t *r, const uint256_t *a, const uint256_t *b) {
    uint64_t c = 0;
    #pragma unroll
    for (int i = 0; i < 8; i++) {
...
__device__ __forceinline__ uint32_t u256_sub(uint256_t *r, const uint256_t *a, const uint256_t *b) {
    int64_t c = 0;
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        c += (int64_t)a->d[i] - (int64_t)b->d[i];
        r->d[i] = (uint32_t)c;
        c >>= 32;
    }
...
__device__ __forceinline__ void mod_reduce(uint256_t *r) {
    // If r >= p, subtract p
...
// r = (a + b) mod p
__device__ __forceinline__ void mod_add(uint256_t *r, const uint256_t *a, const uint256_t *b) {
...
// r = (a - b) mod p
__device__ __forceinline__ void mod_sub(uint256_t *r, const uint256_t *a, const uint256_t *b) {
...
// r = -a mod p
__device__ __forceinline__ void mod_neg(uint256_t *r, const uint256_t *a) {
    if (u256_is_zero(a)) {
        u256_set_zero(r);
    } else {
...
__device__ __forceinline__ void u256_mul_512(uint512_t *r, const uint256_t *a, const uint256_t *b) {
    #pragma unroll
    ...
    for (int i = 0; i < 8; i++) {
        ...
        #pragma unroll
        for (int j = 0; j < 8; j++) {
            uint64_t prod = (uint64_t)a->d[i] * (uint64_t)b->d[j] + (uint64_t)r->d[i+j] + carry;
...
__device__ __forceinline__ void mod_reduce_512(uint256_t *r, const uint512_t *a) {
    // a = a_lo + a_hi * 2^256
    ...
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        uint64_t prod = (uint64_t)high.d[i] * 977ULL + (uint64_t)low.d[i] + carry;
        low.d[i] = (uint32_t)prod;
        carry = prod >> 32;
    }

    // Now add high * 2^32 (shifted by 1 word)
    uint64_t carry2 = 0;
    carry2 += (uint64_t)low.d[1] + (uint64_t)high.d[0];
    low.d[1] = (uint32_t)carry2;
    carry2 >>= 32;

    #pragma unroll
    for (int i = 2; i < 8; i++) {
        carry2 += (uint64_t)low.d[i] + (uint64_t)high.d[i-1];
        low.d[i] = (uint32_t)carry2;
        carry2 >>= 32;
    }

    // Handle overflow from both carries
    uint64_t overflow = carry + carry2 + (uint64_t)high.d[7];
    ...
    while (overflow > 0) {
        ...
        for (int i = 0; i < 8 && (overflow > 0 || c > 0); i++) {
            uint64_t prod = overflow * 977ULL * (i == 0 ? 1 : 0) + (uint64_t)low.d[i] + c;
...
// r = (a * b) mod p
__device__ __forceinline__ void mod_mul(uint256_t *r, const uint256_t *a, const uint256_t *b) {
    uint512_t prod;
...
// r = a^2 mod p
__device__ __forceinline__ void mod_sqr(uint256_t *r, const uint256_t *a) {
    mod_mul(r, a, a);
}
...
__device__ void mod_inv(uint256_t *r, const uint256_t *a) {
    // p - 2 for secp256k1
...
__device__ __forceinline__ void point_set_infinity(Point256 *p) {
    u256_set_zero(&p->x);
    u256_set_zero(&p->y);
    u256_set_zero(&p->z);
}

__device__ __forceinline__ int point_is_infinity(const Point256 *p) {
    return u256_is_zero(&p->z);
}

// Convert affine (x,y) to Jacobian (X,Y,Z=1)
__device__ __forceinline__ void point_from_affine(Point256 *p, const uint256_t *x, const uint256_t *y) {
    u256_set(&p->x, x);
    u256_set(&p->y, y);
    u256_set_u32(&p->z, 1);
}
...
__device__ void point_to_affine(uint256_t *x, uint256_t *y, const Point256 *p) {
...
__device__ void point_get_x_affine(uint256_t *x, const Point256 *p) {
    if (point_is_infinity(p)) {
        u256_set_zero(x);
        return;
    }
...
__device__ void point_double(Point256 *r, const Point256 *p) {
...
__device__ void point_add(Point256 *r, const Point256 *p, const Point256 *q) {
    if (point_is_infinity(p)) {
        *r = *q;
        return;
    }
...
__device__ void point_add_affine(Point256 *r, const Point256 *p, const uint256_t *qx, const uint256_t *qy) {
    if (point_is_infinity(p)) {
...
// Table stored in global memory (uploaded at init)
__device__ Point256 *d_GTable;

// Scalar multiply: R = k * G using precomputed table
__device__ void point_mul_G(Point256 *r, const uint256_t *k) {
    point_set_infinity(r);
...
__device__ void point_add_delta(Point256 *r, const Point256 *p, uint32_t delta) {
    if (delta == 0) {
        *r = *p;
        return;
    }
    ...
    Point256 dG = d_GTable[delta - 1]; // delta*G from first 256 entries
    if (point_is_infinity(p)) {
        *r = dG;
    } else {
        point_add(r, p, &dG);
