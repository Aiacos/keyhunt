...
void adaptive_report_work(worker_type_t worker_type, uint64_t keys_checked, uint64_t elapsed_ms) {
    ...
    /* Use atomic compare-and-swap to ensure only one thread triggers an update.
     * This prevents the race condition where multiple threads could read
    ...
    /* Call update outside the lock - only one thread will reach here per interval */
    if (should_update) {
